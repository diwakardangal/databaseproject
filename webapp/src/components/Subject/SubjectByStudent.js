...
class SubjectByStudent extends Component {
    constructor(props) {
        super(props);
        this.state = {
            scores : [],
                              ...
                              <tbody>
                                {
                                    ...
                                    scores.map((score) => (
                                    score.pupil.id == localStorage.getItem('user_id') ? 
                                    <tr key={score.id}>
                                        <td>{score.id}</td>
                                        <td>{score.pupil.firstName} {score.pupil.lastName}</td>
                                        <td>{score.exam.subject.classes.className}</td>
                                        <td>{score.exam.subject.subjectName}</td>
                                        <td>{score.exam.testName}</td>
                                        <td>{score.score}</td>
                                        <td>
                                            <ButtonGroup>
                                                <Link to={"/editscorebyteacher/"+score.id} className="btn btn-sm btn-outline-primary"><FontAwesomeIcon icon={faEdit} /></Link>{' '}
...
export default connect(mapStateToProps, mapDispatchToProps)(SubjectByStudent);